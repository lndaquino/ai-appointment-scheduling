...
const useAssistant = (ai: AIModel, calendar: ICalendar) => {
  ...
  const generateNewTimeMessage = async (freeTime: string[]): Promise<string> => {
    const prompt = `You're a clinic's secretary helping a customer to schedule an appointment on day ${date}. Generate a ${mood} message up to 30 words with the day ${date} offering at least five of the following available times: ${freeTime}.`
    debug(prompt)
    return await ai.generateMessage(prompt)
  }
  ...
  const checkTimeOnMessage = async (newUserMessage: Message): Promise<string> => {
    ...
    debug(prompt)
    let response = await ai.generateMessage(prompt)
    debug(response)
    response = response.toLowerCase().includes('empty') ? '' : response
  ...
  const generateFinalMessage = async (): Promise<string> => {
    const prompt = `You're a clinic's secretary helping a customer to schedule an appointment. The customer confirmed the appointment on on ${date}, at ${time}. The customer's name is ${confirmedBy}. Generate a ${mood} message up to 20 words confirming the appointment.`
    debug(prompt)
    return await ai.generateMessage(prompt)
  }
